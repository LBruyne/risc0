...
impl<'a, 'b, S: Syscall> Executor<'a, 'b, S> {
    fn ecall_halt(&mut self) -> Result<bool> {
        ...
        println!("ecall_halt({halt_type}, {user_exit})");
    ...
    fn ecall_input(&mut self) -> Result<bool> {
        println!("[{}] ecall_input", self.insn_cycles);
    ...
    fn ecall_software(&mut self) -> Result<bool> {
        println!("[{}] ecall_software", self.insn_cycles);
        ...
        let syscall = if let Some(syscall) = &self.pending.syscall {
            println!("Replay syscall: {syscall:?}");
            syscall.clone()
        } else {
    ...
    fn ecall_sha(&mut self) -> Result<bool> {
        println!("[{}] ecall_sha", self.insn_cycles);
        ...
        // println!("ecall_sha: start state: {state:08x?}");
        ...
        for _ in 0..count {
            ...
            // println!("Compressing block {block:02x?}");
            sha2::compress256(
                &mut state,
        ...
        // println!("ecall_sha: final state: {state:08x?}");
        for word in &mut state {
