...
impl Preflight {
    ...
    fn post_steps(&mut self) -> Result<()> {
        ...
        let body_padding = max_cycles
            ...
            .ok_or_else(err)?;
        println!("padding: {body_padding}, pre: {pre_cycles}, body: {body_cycles}");
...
impl Preflight {
    ...
    fn ecall_bigint(&mut self) -> Result<bool> {
        ...
        println!("bigint(z: {z_ptr:?}, x: {x_ptr:?}, y: {y_ptr:?}, n {n_ptr:?}");
