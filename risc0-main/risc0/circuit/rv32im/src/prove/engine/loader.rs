...
impl<'a> Loader<'a> {
    ...
    fn body(&mut self) {
        ...
        println!("[{}] BODY: {body_cycles}", self.cycle);
    ...
    fn bytes_init(&mut self) {
        println!("[{}] BYTES_INIT", self.cycle);
        self.add_cycle(CtrlCycle::bytes_init());
    }

    fn bytes_setup(&mut self) {
        println!("[{}] BYTES_SETUP", self.cycle);
    ...
    fn reset(&mut self, phase: u32) {
        println!("[{}] RESET({phase})", self.cycle);
    ...
    fn fini(&mut self) {
        println!("[{}] RAM_FINI", self.cycle);
        self.add_cycle(CtrlCycle::ram_fini());
        println!("[{}] BYTES_FINI", self.cycle);
