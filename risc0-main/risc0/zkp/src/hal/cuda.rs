...
impl CudaHash for CudaHashSha256 {
    ...
    fn hash_fold(&self, hal: &CudaHal<Self>, io: &BufferImpl<Digest>, output_size: usize) {
        ...
        unsafe {
            ...
            let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
            let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
            println!("1：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
    ...
    fn hash_rows(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("2：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
...
impl CudaHash for CudaHashPoseidon {
    ...
    fn hash_fold(&self, hal: &CudaHal<Self>, io: &BufferImpl<Digest>, output_size: usize) {
        ...
        unsafe {
            ...
            let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
            let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
            println!("3：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
    ...
    fn hash_rows(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("4：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                self.round_constants.as_device_ptr(),
                self.mds.as_device_ptr(),
...
impl CudaHash for CudaHashPoseidon2 {
    ...
    fn hash_fold(&self, hal: &CudaHal<Self>, io: &BufferImpl<Digest>, output_size: usize) {
        ...
        unsafe {
            ...
            let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
            let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
            println!("5：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                self.round_constants.as_device_ptr(),
    ...
    fn hash_rows(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("6：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                self.round_constants.as_device_ptr(),
                self.m_int_diag.as_device_ptr(),
...
impl<CH: CudaHash> Hal for CudaHal<CH> {
    ...
    fn batch_bit_reverse(&self, io: &Self::Buffer<Self::Elem>, count: usize) {
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("7：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                io.as_device_ptr(),
                bits,
    ...
    fn batch_evaluate_any(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("8：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
    ...
    fn gather_sample(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("9：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
    ...
    fn mix_poly_coeffs(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("10：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                output.as_device_ptr(),
                input.as_device_ptr(),
    ...
    fn eltwise_add_elem(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("11：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                output.as_device_ptr(),
    ...
    fn eltwise_sum_extelem(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("12：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                output.as_device_ptr(),
                input.as_device_ptr(),
    ...
    fn eltwise_copy_elem(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("13：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                output.as_device_ptr(),
                input.as_device_ptr(),
    ...
    fn fri_fold(
        ...
        let millisecond = local_time.nanosecond() / 1_000_000; // 将纳秒转换为毫秒
        let nanosecond = local_time.nanosecond() % 1_000_000; // 获取剩余的纳秒
        println!("14：Current time: {:02}:{:02}:{:02}.{:03}.{:06}",hour, minute, second, millisecond, nanosecond);
        unsafe {
            launch!(kernel<<<params.0, params.1, 0, stream>>>(
                output.as_device_ptr(),
                input.as_device_ptr(),
