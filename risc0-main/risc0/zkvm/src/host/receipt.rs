...
impl Receipt {
    ...
    pub fn verify_with_context(
        ...
        println!("Receipt::verify_with_context");
        ...
        if claim.output.digest() != expected_output.digest() {
            ...
            println!("accepting zero digest for output of receipt with empty journal");
        }
    ...
    pub fn verify_integrity_with_context(
        ...
        println!("Receipt::verify_integrity_with_context");
        self.inner.verify_integrity_with_context(ctx)?;
...
impl InnerReceipt {
    ...
    pub fn verify_integrity_with_context(
        ...
        println!("InnerReceipt::verify_integrity_with_context");
        match self {
...
impl CompositeReceipt {
    ...
    pub fn verify_integrity_with_context(
        ...
        println!("CompositeReceipt::verify_integrity_with_context");
        ...
        println!("final: {:#?}", final_receipt.claim);
        if let Some(id) = expected_pre_state_digest {
        ...
        for receipt in self.assumptions.iter() {
            println!("verifying assumption: {:?}", receipt.get_claim()?.digest());
            receipt.verify_integrity_with_context(ctx)?;
        }
    ...
    fn verify_output_consistency(&self, claim: &ReceiptClaim) -> Result<(), VerificationError> {
        ...
        if claim.exit_code.expects_output() && claim.output.is_some() {
            ...
            if claim.output.is_some() {
                println!("unexpected non-empty claim output: {:?}", &claim.output);
                return Err(VerificationError::ReceiptFormatError);
            }
...
impl SegmentReceipt {
    ...
    pub fn verify_integrity_with_context(
        ...
        println!("SegmentReceipt::verify_integrity_with_context");
        use hex::FromHex;
