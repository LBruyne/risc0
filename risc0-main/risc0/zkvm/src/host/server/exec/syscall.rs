...
impl Syscall for SysRandom {
    fn syscall(
        ...
        println!("SYS_RANDOM: {}", to_guest.len());
...
impl SysVerify {
    ...
    fn sys_verify_integrity(&mut self, from_guest: Vec<u8>) -> Result<(u32, u32)> {
        ...
        println!("SYS_VERIFY_INTEGRITY: {}", hex::encode(&claim_digest));
    ...
    fn sys_verify_cmp(
        ...
        if !claim.as_value()?.exit_code.is_ok() {
            println!("sys_verify: ignoring matching claim with error exit code: {claim:?}");
            return Ok(None);
        };
